...
router.get('/', async (req, res) => {
    try {
        ...
        let selectColumns = '*, COALESCE(NULLIF(new_slug, ""), slug) as slug';
        if (req.query.summary) {
            selectColumns = 'id, title, COALESCE(NULLIF(new_slug, ""), slug) as slug, image, category, upcoming, status, gmp_date, gmp_ipo_price, gmp, created_at';
        }
...
router.get('/:slug', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT *, COALESCE(NULLIF(new_slug, ""), slug) as slug FROM admin_blogs WHERE new_slug = ? OR slug = ? OR id = ? LIMIT 1', [req.params.slug, req.params.slug, req.params.slug]);
