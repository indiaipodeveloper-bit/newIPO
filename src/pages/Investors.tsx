...
const Investors = () => {
  ...
  const validateMobile = (mobile: string) => {
    // Basic Indian mobile validation: 10 digits starting with 6-9
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (countWords(formData.ticket_size) > 30) {
      toast.error("Ticket size should not exceed 30 words.");
      return;
    }
    if (countWords(formData.industry) > 30) {
      toast.error("Industry should not exceed 30 words.");
      return;
    }
    if (countWords(formData.roi) > 40) {
      toast.error("Expected ROI should not exceed 40 words.");
      return;
    }
    if (countWords(formData.tenure) > 15) {
      toast.error("Investment tenure should not exceed 15 words.");
      return;
    }
    if (countWords(formData.inv_type) > 15) {
      toast.error("Investment type should not exceed 15 words.");
      return;
    }
    if (countWords(formData.buss_type) > 15) {
      toast.error("Business type should not exceed 15 words.");
      return;
    }
    if (countWords(formData.query) > 150) {
      toast.error("Message should not exceed 150 words.");
      return;
    }
        ...
        <section id="benefits" className="py-24">
          <div className="container mx-auto px-4">
            ...
            <div className="overflow-hidden md:overflow-visible pb-8 -mx-4 px-4 scrollbar-hide">
              <div className="animate-marquee-mobile flex gap-6 md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-8 hover:[animation-play-state:paused]">
                {benefits.map((item, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      transition={{ delay: idx * 0.1 }}
                      viewport={{ once: true }}
                      className="w-[280px] md:w-auto group bg-card border border-border rounded-2xl p-8 hover:shadow-xl hover:border-primary/40 transition-all duration-300 relative overflow-hidden"
                    >
                ...
                {benefits.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={`dup-${idx}`}
                      className="md:hidden w-[280px] group bg-card border border-border rounded-2xl p-8 transition-all duration-300 relative overflow-hidden"
                    >
                      <div className="relative z-10">
